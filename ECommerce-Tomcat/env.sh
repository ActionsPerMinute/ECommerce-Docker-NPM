...
if [ -n "${orders}" ]; then
		if [ -z "${NODE_NAME}" ]; then
			export NODE_NAME="Node_OrderProcessor";
		fi
		
		if [ -z "${TIER_NAME}" ]; then
			export TIER_NAME="OrderProcessor";
		fi
fi

if [ -n "${payments}" ]; then
		if [ -z "${NODE_NAME}" ]; then
			export NODE_NAME="Node_PaymentGateway";
		fi
		
		if [ -z "${TIER_NAME}" ]; then
			export TIER_NAME="PaymentGateway";
		fi
fi
...
export APP_AGENT_JAVA_OPTS="${JAVA_OPTS} ${APPD_JAVA_OPTS} -DjvmRoute=${JVM_ROUTE} -Djava.util.logging.manager=org.apache.juli.ClassLoaderLogManager -Dappdynamics.agent.accountName=customer1 -Dappdynamics.agent.accountAccessKey=${ACCESS_KEY}";
